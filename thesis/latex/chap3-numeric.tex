...
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
\قسمت{بدون بعد سازی معادلات}
ابتدا برای ساده‌تر شدن کار با معادلات آن‌ها را بی‌بعد می‌سازیم، به همین منظور مقادیر مرجع طول $L^*$، فشار $P^*$، زمان $T^*$، تراوایی مطلق $K^*$ و سرعت $u^*$ را تعریف می‌کنیم. حال می‌توانیم پارامتر‌های بدون بعد را به صورت معادله \ارجاع‌فر{eq:3dim1} تعریف کنیم:
\begin{equation}
\label{eq:3dim1}
...
حال با جایگذاری مقادیر بی‌بعد در معادلات \ارجاع‌فر{eq:2phic}، \ارجاع‌فر{eq:2comp1}--\ارجاع‌فر{eq:2comp4} خواهیم داشت:
\begin{align}
\label{eq:3dim2}
&\tilde{\nabla}\cdot\left[
...
$(.) \doteq \tilde{(.)}$.
معادلات \ارجاع‌فر{eq:3dim2}--\ارجاع‌فر{eq:3dim6} اساس کار ما را تشکیل خواهند داد و برای تعریف یک مسئله کافی است پارامتر‌های $\tilde{\phi}$، $\mathcal{M}$، $\mathcal{N}$، $\mathcal{P} $ و $\mathcal{G}$، هندسه با طول بی‌بعد و شرایط مرزی (حالت بدون بعد معادلات \ارجاع‌فر{eq:2bndout}) داده شوند.
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
...
%----------------------------------------------------------------------------------------ی
\قسمت{مش مورد نیاز}
برای حل معادلات \ارجاع‌فر{eq:3dim2}--\ارجاع‌فر{eq:3dim6}، اولین قدم مش زدن هندسه است. مش مورد نیاز باید دو شرط مهم را داشته باشد:
\begin{tight_enumerate}
\فقره اضلاع المان‌ها مرز محیط‌ها و ترک‌ها را قطع نکنند، یعنی نود‌ها روی مرز نواحی و روی ترک‌ها قرار گیرند\پانویس{Conforming Mesh}.
\فقره مش از المان‌های مثلثی، چهار‌ضلعی‌ و خطی تشکیل شده باشد و المان‌های مثلثی و چهارضلعی متعلق به نواحی ماتریس و المان‌های خطی مربوط به نواحی ترک باشند.
\end{tight_enumerate}
در شکل \رجوع{fig:3mesh} یک نمونه مش مناسب برای شبیه‌سازی عددی را مشاهده می‌کنید. در این مش خطوط نازک المان‌های ماتریس و خطوط کلفت المان ترک یا مرز نواحی ماتریس را نشان می‌دهند.

\begin{figure}
\includegraphics[width=.6\textwidth,center]{numeric-mesh.eps}
\شرح{مش مناسب برای شبیه‌سازی عددی}
\label{fig:3mesh}
\end{figure}
...
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
\قسمت{معرفی نوتاسیون استفاده شده در روش عددی}
در این قسمت نوتاسیون مورد نیاز برای بیان روش عددی را بیان خواهیم کرد. به همین منظور در شکل \رجوع{fig:3not} یک نود به نام $i$ یا $v_i$ و المان‌ها و نود‌های اطراف آن نمایش داده شده‌اند. همانطور که مشاهده می‌کنید امکان دارد که اطراف یک نود المان‌های ترک مثل المان $C$ یا $e_C$ نیز وجود داشته باشند. علاوه بر نمایش نام‌گذاری المان‌ها و نود‌ها در شکل \رجوع{fig:3not-mesh}، در شکل \رجوع{fig:3not-geo} تعدادی نقطه مهم از جمله وسط اضلاعی که نود $v_i$ عضوی از آنهاست مثل $\frac{ij}{2}$ و مراکز هندسی المان‌ها مثل $G_B$ نمایش داده شده‌اند. 

\begin{figure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=.95\linewidth,center]{numeric-notation-geo.jpg} 
\شرح{نقاط کمکی برای معرفی حجم کنترل و بردارها}
\label{fig:3not-geo}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
...
\شرح{مثالی از یک نود به نام $v_i$ و اجزای اطرافش}
\label{fig:3not}
\end{figure}

اولین مفهوم مهم در مش اضلاع هندسی\پانویس{Edge} هستند. به اضلاع المان‌ها، اضلاع هندسی گفته می‌شوند. برای مثال در شکل \رجوع{fig:3not} خطوط $ij$ یا $ik$ اضلاع هندسی هستند. مفهوم مهم دیگر احجام کنترل هستند. با روشی که ذکر خواهیم کرد متناظر با هر نود یک حجم کنترل خواهیم ساخت: نقاط وسط اضلاع هندسی اطراف هر نود را به مرکز حجم المان متناظرش متصل می‌کنیم. چند ضلعی حاصله حجم کنترل متناظر با نود مذکور خواهد بود. برای مثال در شکل \رجوع{fig:3not} حجم کنترل متناظر با نود $i$، چند ضلعی 
$G_BG_CG_D\frac{io}{2}G_E\frac{ij}{2}G_A\frac{ik}{2}$
است. مفهوم بعدی اضلاع دوگانه\پانویس{Dual-edge} است. به هر یک از اضلاع احجام کنترل یک ضلع دوگانه می‌گوییم. با دقت در شکل \رجوع{fig:3not} می‌توان مشاهده کرد که متناظر با هر ضلع هندسی به تعداد المان‌هایی که این ضلع هندسی عضو آن‌هاست، ضلع دوگانه وجود دارد. یعنی به ازای هر ضلع هندسی و هر المان یک ضلع دوگانه وجود دارد. برای مثال ضلع دوگانه 
$G_E\frac{ij}{2}$
...
مفهوم مهم دیگر المثنی‌ها\پانویس{Duplicates} هستند. از بخش \رجوع{ch:253} می‌دانیم که در هر نود که روی مرز چند ناحیه قرار گیرد، چند مقدار اشباع باید ذخیره شود. به همین منظور مفهومی به نام المثنی‌ها را معرفی می‌کنیم. به ازای هر نود در مش به تعداد نواحی احاطه کننده آن، المثنی وجود خواهد داشت. المثنی‌ها اشیاء فرضی هستند که مقادیری مثل اشباع را به آن‌ها نسبت خواهیم داد. برای مثال در شکل \رجوع{fig:3not} سه ناحیه مختلف (دو ناحیه ماتریس و یک ناحیه ترک) نود $i$ را احاطه کرده‌اند. لذا سه المثنی متناظر با این نود وجود خواهد داشت. در این شکل این سه المثنی را
$d_1$، $d_2$ و $d_3$
نامیده‌ایم. مطابق دستورالعملی که در فصل \رجوع{ch:fasl2} بیان شد، یکی از این المثنی‌ها ارباب و بقیه برده‌ خواهند بود و مقدار اشباع برده‌ها وابسته به مقدار اشباع در ارباب خواهد بود. برخلاف اشباع، مقادیری مثل پتانسیل آب $\varphi_w$ بنابر معادله \ارجاع‌فر{eq:2bndin} به نواحی بستگی ندارند، لذا به نود‌ها نسبت داده ‌می‌شوند.

دیگر مفاهیم مورد نیاز، به همراه مثالی از شکل \رجوع{fig:3not} به شرح زیر هستند:  
\begin{tight_itemize}
\item[$v_i$]
نود شماره $i$
\item[$e_i$]
...
\item[$\chi|_{g_{ij}^k}$]
مقدار متغیر $\chi$ در نقطه $g_{ij}^k$ که به کمک روش \متن‌لاتین{upwind} و از درون‌یابی مقادیر $\chi|_{di}$ پیدا می‌شود. متغیرهای
$k_{rw}$ و $k_{ro}$
می‌توانند به جای $\chi$ قرار گیرند. 
\item[$\nabla\chi|_{g_{ij}^k}$]
...
را بیان می‌کنیم. برای محاسبه $k_{ro}|_{g_{ij}^k}$ و $k_{rw}|_{g_{ij}^k}$ از روش \متن‌لاتین{upwind} استفاده می‌نماییم. این روش در معادله \ارجاع‌فر{eq:3up} بیان شده است.
\begin{equation}
\label{eq:3up}
...
\quad \alpha = w,o
\end{equation}
در این معادله مقادیر $u_\alpha$ باید از معادلات \ارجاع‌فر{eq:3dim4}، \ارجاع‌فر{eq:3dim5} محاسبه شوند.

برای محاسبه  $\nabla\varphi_\alpha|_{g_{ij}^k}$  از توابع شکل\پانویس{Shape Functions} استفاده می‌کنیم. به این منظور در هر المان برای هر نود یک تابع شکل تعریف می‌کنیم: $\psi_i^k$ یعنی تابع شکل نود $v_i$ در المان $e_k$. ویژگی تابع شکل 
...
\end{equation}
با تعریف توابع شکل می‌توانیم توزیع متغیر $\varphi_w$ را به صورت معادله \ارجاع‌فر{eq:3sf2} در هر المان باز‌سازی کنیم:
\begin{equation}
\label{eq:3sf2}
...
لذا با مشتق‌گیری از معادله  \ارجاع‌فر{eq:3sf2} و قرار دادن مختصات نقاط $g_{ij}^k$ به جای $\vec X$ مقدار $\nabla\varphi_w|_{g_{ij}^k}$ محاسبه می‌شود:
\begin{equation}
\label{eq:3sf3}
...
تنها تفاوت معادلات  \ارجاع‌فر{eq:3sf3} و  \ارجاع‌فر{eq:3sf4} این است که مقادیر پتانسیل آب در نود‌ها ذخیره می‌شوند ولی مقادیر پتانسیل مویینگی در المثنی‌ها ذخیره می‌شوند.
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
%                                      قسمت چهارم: روش IMPES
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
\قسمت{روش \متن‌لاتین{IMPES}}
در این قسمت توضیح خواهیم داد که در هر گام زمانی چه معادلاتی باید حل شوند و مجهولات معادلات چه هستند.

هدف در این پروژه پیدا کردن مقادیر $S|_{di}$ و $\varphi|_{vi}$ در هر گام زمانی می‌باشد، اما همانطور که در  بخش  \رجوع{ch:253} نشان دادیم، مقادیر
 $S|_{d_i} \quad d_i\in\gamma(v_j)$
 مستقل از یکدیگر نیستند و همگی تابع  $S|_{d_j^*}$  هستند. لذا مجهولات خود را دو بردار 
$\vec{\Phi}^k$ و $\vec{S}^k$
تعریف می‌کنیم، که به ترتیب مقادیر $\varphi|_{v_i}$ و $S|_{d_i^*}$ را در گام زمانی $k$ام نشان می‌دهند. طول هر دو بردار نیز برابر \متن‌لاتین{NV} می‌باشد.

فرض کنید در گام زمانی $k$ مقدار $\vec{S}^k$ مشخص است. روش \متن‌لاتین{IMPES} برای پیدا کردن دو بردار 
$\vec{\Phi}^k$ و $\vec{S}^{k+1}$
فرآیند ذیل را پیشنهاد می‌دهد:
\begin{tight_enumerate}
\فقره با گسسته سازی معادله \ارجاع‌فر{eq:3dim2} مقدار $\vec{\Phi}^k$ را محاسبه کنید. به این منظور باید دستگاهی به صورت زیر را حل نمایید:
\begin{equation}
\label{eq:3impes1}
\begin{aligned}
...
\end{aligned}
\end{equation}
در واقع نام فشار ضمنی به این خاطر به این روش داده شده‌است که پتانسیل آب در معادله \ارجاع‌فر{eq:3impes1} به صورت ضمنی حل می‌شود. همانطور که مشاهده می‌کنید، طرف چپ و راست این معادله صرفاً تابعی از اشباع در بازه زمانی $k$ و شرایط مرزی پتانسیل آب هستند، لذا معادله کاملاً خطی خواهد بود.
\فقره سپس با گسسته سازی معادله \ارجاع‌فر{eq:3dim3} مقدار $\vec{S}^{k+1}$ را محاسبه کنید. به این منظور باید معادله‌ای به صورت زیر را حل نمایید:
\begin{equation}
\label{eq:3impes2}
\begin{aligned}
...
\end{aligned}
\end{equation}
در این معادله اشباع در گام زمانی $k+1$ به صورت کاملاً صریح حل شده است و برخلاف معادله \ارجاع‌فر{eq:3impes1} دیگر یک دستگاه نیست. همین عمل است که باعث خطی شدن معادله می‌شود، امّا برای همگرا شدن پاسخ‌ها باید گام زمانی کوچک انتخاب شود. 
\end{tight_enumerate}
...
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
\قسمت{معادله حجم محدود}
\label{ch:35}
در این قسمت روش حجم محدود را برای محاسبه بردار‌های 
$\vec{\mathcal B}_G$، $\vec{\mathcal C}_G$، $\vec{\mathcal F}_G$ و ماتریس $\textbf{A}_G$
معرفی خواهیم کرد. روش حجم محدود استفاده شده \متن‌لاتین{CVFEM}\پانویس{Control Volume Finite Element} نام دارد و اوّلین بار در \مرجع{baliga} برای حل معادله \متن‌لاتین{advection-diffusion} به کار رفته‌است. نحوه تعمیم این روش برای حل معادلات جریان دوفازی در محیط متخلخل از  \مرجع{mont1} گرفته شده‌است. 
%--------------------------------------------۳۵۱ معادله پتانسیل------------------------------ی
\زیرقسمت{معادله پتانسیل}
ابتدا نحوه ایجاد معادله \ارجاع‌فر{eq:3impes1} را از معادله \ارجاع‌فر{eq:3dim2} بیان خواهیم کرد. به این منظور ابتدا از معادله \ارجاع‌فر{eq:3dim2} در هر حجم کنترل $B_i$ انتگرل می‌گیریم و از قضیه دیورژانس برای انتقال انتگرال‌های حجمی به مرز استفاده می‌کنیم:
\begin{equation}
\label{eq:3fvmp1}
...
برای بیان تقریب عددی معادله \ارجاع‌فر{eq:3fvmp1} ابتدا تقریب عددی کل شار عبوری از نود $v_i$ به سمت نود $v_j$ در المان $e_k$ را به صورت زیر تعریف می‌کنیم:
\begin{equation}
\label{eq:3fvmp2}
\begin{aligned}
-\text{TFlux}_{ij}^k 
...
از حالت اوّل برای المان‌های داخلی و از حالت دوم برای المان‌های مجازی (اعمال شروط مرزی) استفاده خواهیم کرد. حال تقریب عددی معادله \ارجاع‌فر{eq:3fvmp1} به صورت زیر در خواهد آمد:
\begin{equation}
\label{eq:3fvmp3}
...
معادله \ارجاع‌فر{eq:3fvmp3} به این معنی است که جمع جبری کل شار ورودی و خروجی به هر حجم کنترل برابر صفر باشد. حال برای اینکه بتوان معادلات \ارجاع‌فر{eq:3fvmp3} را به شکل دستگاه معادلات \ارجاع‌فر{eq:3impes1} در آورد و ماتریس $\textbf{A}_G$ و بردار $\vec{\mathcal B}_G$ را از آن استخراج نمود، فرض می‌کنیم که $\text{TFlux}_{ij}^k$ در هر المان تابعی خطی از مقدار پتانسیل آب در نود‌های عضو المان باشد، یعنی:
\begin{equation}
\label{eq:3fvmp4}
...
حال نشان می‌دهیم که در صورت وچود مقادیر $a$ و $b$ چگونه به کمک آن‌ها ماتریس $\textbf{A}_G$ و بردار $\vec{\mathcal B}_G$ را بسازیم. خود اینکه مقادیر $a$ و $b$ را چگونه پیدا کنیم را در بخش \رجوع{ch:36} ذکر خواهیم کرد. اگر معادله \ارجاع‌فر{eq:3fvmp4} را در معادله \ارجاع‌فر{eq:3fvmp3} جایگذاری کنیم، خواهیم داشت:
\begin{equation}
\label{eq:3fvmp5}
\begin{aligned}
&\sum_{e_k\in\theta(v_i)} \sum_{v_j\in\eta(v_i,e_k)} -\text{TFlux}_{ij}^k = 0 \\
...
از مقایسه معادلات \ارجاع‌فر{eq:3fvmp6} و \ارجاع‌فر{eq:3impes1} درمی‌یابیم که:
\begin{align}
\label{eq:3fvmp7}
&\textbf{A}_{Gij} = 
\begin{cases}
...
معادلات \ارجاع‌فر{eq:3fvmp7} و \ارجاع‌فر{eq:3fvmp8} به این معنا هستند که ماتریس‌ و بردار جهانی  $\textbf{A}_G$ و $\vec{\mathcal B}_G$ را می‌توان از اسمبل کردن ماتریس‌‌های محلّی $\textbf{A}^k$ و بردارهای محلّی $\vec{\mathcal B}^k$ برای هر المان محاسبه کرد. این ماتریس‌ها برای المان شماره $k$ به صورت معادلات \ارجاع‌فر{eq:3fvmp9} و \ارجاع‌فر{eq:3fvmp10} تعریف شده‌اند. البته در ماتریس‌های محلّی برای سادگی از شماره‌گذاری محلّی استفاده می‌کنیم.
\begin{align}
\label{eq:3fvmp9}
...
\end{align} 
%--------------------------------------------۳۵۲ معادله اشباع------------------------------ی
\زیرقسمت{معادله اشباع}
\label{ch:352}
در این قسمت نحوه ایجاد معادله \ارجاع‌فر{eq:3impes2} را از معادله \ارجاع‌فر{eq:3dim3} بیان خواهیم کرد. مشابه حالت قبلی از معادله \ارجاع‌فر{eq:3dim3} در هر حجم کنترل $B_i$ انتگرل می‌گیریم و از قضیه دیورژانس برای انتقال انتگرال‌های حجمی به مرز استفاده می‌کنیم:
\begin{equation}
\label{eq:3fvms1}
...
برای محاسبه تقریب عددی طرف راست معادله \ارجاع‌فر{eq:3fvms1}، تقریب عددی شار آب عبوری از نود $v_i$ به سمت نود $v_j$ در المان $e_k$ را به صورت زیر تعریف می‌کنیم:
\begin{equation}
\label{eq:3fvms2}
\begin{aligned}
-\text{SFlux}_{ij}^k 
...
حال تقریب عددی طرف راست معادله \ارجاع‌فر{eq:3fvms1} به صورت زیر در می‌آید:
\begin{equation}
\label{eq:3fvms4}
\int_{\partial B_i} 
k_{rw} \textbf{K} \nabla \varphi_w \cdot \vec{n} d\Gamma 
...
معادله \ارجاع‌فر{eq:3fvms4} به این معناست که بردار جهانی $\vec{\mathcal F}_G$ را می‌توان از اسمبل کردن بردار‌های محلی $\vec{\mathcal F}^k$ برای هر المان محاسبه کرد. این بردار برای المان شماره $k$ به صورت معادله \ارجاع‌فر{eq:3fvms5} تعریف شده‌است. 
\begin{equation}
\label{eq:3fvms5}
\vec{\mathcal F}^k = \left[ f_i^k \right] \qquad i= 1,\ldots,N^k,  \qquad  k=1,\ldots,\text{NE}
\end{equation}

نحوه محاسبه مقادیر $f$ را در بخش  \رجوع{ch:36} بیان خواهیم کرد. اکنون تقریب عددی طرف چپ معادله \ارجاع‌فر{eq:3fvms1} را بیان خواهیم کرد. به این منظور از روش \متن‌لاتین{forward Euler} برای تقریب مشتق اشباع در زمان استفاده می‌کنیم:
\begin{equation}
\label{eq:3fvms6}
...
همانطور که در بخش  \رجوع{ch:253} نشان دادیم، مقادیر اشباع برده تابعی از مقادیر اشباع ارباب هستند، لذا با فرض کوچک بودن $\triangle t$ می‌توانیم بنویسیم:
\begin{equation}
\label{eq:3fvms7}
...
مقدار $\frac{d S|_{d_i^j}}{d S|_{d_i^*}}$ از معادلات معرفی شده در بخش  \رجوع{ch:253} به دست می‌آید. با جایگذاری معادله \ارجاع‌فر{eq:3fvms7} در \ارجاع‌فر{eq:3fvms6} خواهیم داشت:
\begin{equation}
\label{eq:3fvms8}
...
از مقایسه معادله \ارجاع‌فر{eq:3fvms8} و \ارجاع‌فر{eq:3impes2} آرایه‌های بردار 
$\vec {\mathcal{C}}_G$
نیز مطابق معادله \ارجاع‌فر{eq:3fvms9} به دست می‌آیند.
\begin{equation}
\label{eq:3fvms9}
\vec {\mathcal{C}}_{Gi} = 
...
\زیرقسمت{المان ترک}
در شکل \رجوع{fig:3frac} یک المان ترک را مشاهده می‌کنید. برای محاسبه مقادیر 
...
\شرح{المان ترک و نام‌گذاری‌های محلّی}
\label{fig:3frac}
\end{figure}
...
حال توابع شکل را به صورت معادله \ارجاع‌فر{eq:3frac1} تعریف می‌نماییم:
\begin{equation}
\label{eq:3frac1}
...
از ترکیب معادلات \ارجاع‌فر{eq:3sf3}،  \ارجاع‌فر{eq:3sf4} و \ارجاع‌فر{eq:3frac1}  نتیجه می‌شود که مقدار گرادیان پتانسیل‌ها برابر خواهد بود با:
\begin{equation}
\label{eq:3frac2}
...
اکنون می‌توانیم به کمک مقادیر گرادیان پتانسیل‌ها جهت سرعت هر فاز را تعیین کرده و از روش \متن‌لاتین{upwind} مقادیر
$\lambda_\alpha|_g$
را بیابیم. اگر معادلات \ارجاع‌فر{eq:3dim4}، \ارجاع‌فر{eq:3dim5}، \ارجاع‌فر{eq:3up} و \ارجاع‌فر{eq:3frac2}  را ترکیب کنیم، خواهیم داشت:
\begin{equation}
\label{eq:3frac3}
\lambda_w|_g = 
\begin{cases}
...
اکنون مقادیر 
$a_{11}$، $a_{12}$ و $b_1$
را محاسبه می‌نماییم. از معادله \ارجاع‌فر{eq:3fvmp4}، داریم:
\begin{equation}
\label{eq:3frac4}
-\text{TFlux}_{12} = a_{11}\varphi_w|_1 + a_{11}\varphi_w|_2 + b_1
\end{equation}

از طرفی از ترکیب معادلات \ارجاع‌فر{eq:3fvmp2} و \ارجاع‌فر{eq:3frac2} داریم:
\begin{equation}
\label{eq:3frac5}
\begin{aligned}
-\text{TFlux}_{12} 
...
از مقایسه \ارجاع‌فر{eq:3frac4} و \ارجاع‌فر{eq:3frac5} واضح است که:
\begin{equation}
\label{eq:3frac6}
...
\شرح{المان‌های مثلث و چهارضلعی و المان‌های مرجع در فضای $\xi \eta$}
\label{fig:3poly}
\end{figure}
%------x

در شکل \رجوع{fig:3poly} یک المان مثلث و یک المان چهارضلعی در فضای  $xy$ و المان‌های مرجع متناظرشان  در فضای  $\xi \eta$ نشان داده شده‌اند.  همانطور که مشاهده می‌کنید، مثلث مرجع یک مثلث قائم‌الزاویه و چهارضلعی مرجع یک مربع می‌باشند. مختصات رئوس آن‌ها در جدول \رجوع{tab:3poly} آمده است. نکته مهم این است که نام‌گذاری نود‌ها در هر دو فضا باید پادساعتگرد باشد. اکنون تعاریف جدید مورد نیاز را ارائه می‌دهیم.

%------x
\begin{table}
\centering
\شرح{مختصات نود‌های اصلی، نقاط $V_i$ و بردار‌های $\vec \Delta_i$ در فضای $\xi \eta$ }
\label{tab:3poly}
...
\زیرزیرقسمت*{نقاط انتگرال‌گیری حجم}
نقطه مرکز هندسی زیر حجم کنترل $B_i$ در فضای $\xi \eta$ را نقطه انتگرال‌گیری حجم یا $V_i$ می‌نامیم. مختصات این نقطه در المان‌های چهارضلعی برای محاسبه حجم $B_i$ استفاده می‌شوند ولی در المان‌های مثلثی نیازی به آن‌ها نیست. مختصات این نقاط برای المان مرجع مربع در جدول\رجوع{tab:3poly} آمده‌ است.

\زیرزیرقسمت*{ماتریس توابع شکل}
ماتریس  توابع شکل
 $ \mathbf{\Psi}_{1 \times N} $
  را به صورت 
$\left[ \psi_1 \quad \ldots \quad \psi_N \right]$
تعریف می‌کنیم. این ماتریس برای المان‌ها به صورت معادله \ارجاع‌فر{eq:3poly2} خواهد بود.
\begin{equation}
\label{eq:3poly2}
\begin{aligned}
...
\زیرزیرقسمت*{ماتریس مشتق توابع شکل}
...
اگر معادله \ارجاع‌فر{eq:3poly2} را در معادله \ارجاع‌فر{eq:3poly3} جایگذاری کنیم، ماتریس مشتق توابع شکل برای المان‌های مثلث و چهار‌ضلعی به دست می‌آید:
\begin{equation}
\label{eq:3poly4}
...
با ترکیب معادلات \ارجاع‌فر{eq:3poly1}، \ارجاع‌فر{eq:3poly2} و استفاده از قاعده زنجیره‌ای می‌توان نشان داد که ماتریس یاکوبی را می‌توان از معادله زیر محاسبه کرد:
\begin{equation}
\label{eq:3poly8}
\textbf J =  \textbf X \frac{\partial \mathbf \Psi}{\partial \vec \xi} 
\end{equation}

\زیرزیرقسمت*{پارامتر‌های مربوط به اضلاع دوگانه}
مقادیر 
$\vec n_i$ ، $g_i$ و $l_i$
به ترتیب متناظر بردار یکّه نرمال، نقطه وسط و طول اضلاع دوگانه داخل المان هستند. نحوه نام‌گذاری اضلاع دوگانه و جهت بردار‌های $\vec n_i$ در شکل \رجوع{fig:3poly} نشان داده شده‌است. برای مثال برای المان مثلث رابطه بردارهای $\vec n_i$ و $\vec n_{ij}$ به صورت 
$\vec n_1 = \vec n_{13}$ ، $\vec n_2 = \vec n_{21}$ و $\vec n_3 = \vec n_{32}$
می‌باشد.

\زیرزیرقسمت*{بردارهای $\vec \Delta$}
در فضای $\xi \eta$ بردار‌ $\vec \Delta$ را برداری تعریف می‌کنیم که هم‌راستا با ضلع دوگانه‌ای است که نقطه وسط آن $g_i$ باشد و جهت آن به سمت داخل المان باشد. در شکل \رجوع{fig:3poly} راستا و جهت و در جدول \رجوع{tab:3poly} مختصات این بردار‌ها برای المان‌های مرجع مثلث و مربع نشان داده‌ شده‌اند. از رابطه \ارجاع‌فر{eq:3poly7x} می‌توان ثابت کرد:
\begin{equation}
\label{eq:3poly9}
...
با استفاده از روابط \ارجاع‌فر{eq:3poly9} و \ارجاع‌فر{eq:3poly7xx} می‌توان ثابت کرد که ماتریس $\textbf H_i$ را می‌توان از رابطه زیر محاسبه کرد:
\begin{equation}
\label{eq:3poly12}
\textbf H_i = \left( 
...
با کمک تعاریفی که ارائه دادیم می‌توانیم محاسبه ماتریس‌های محلّی را شروع کنیم. ابتدا نحوه محاسبه مقادیر \متن‌لاتین{upwind} را نشان می‌دهیم. با استفاده از روابط \ارجاع‌فر{eq:3up} و \ارجاع‌فر{eq:3poly11} می‌توان نشان داد که مقادیر $\lambda|_{g_i}$ را  می‌توان از روابط زیر محاسبه نمود:
\begin{equation}
\label{eq:3poly13}
\lambda_w|_{g_i} = 
	\begin{cases}
...
اکنون نحوه محاسبه مقادیر $a$ و $b$ را برای نود شماره ۱ نشان می‌دهیم. به این منظور از رابطه \ارجاع‌فر{eq:3fvmp4} داریم:
\begin{equation}
\label{eq:3poly14}
...
از طرفی از ترکیب معادلات  \ارجاع‌فر{eq:3fvmp2}  داریم :
\begin{equation}
\label{eq:3poly15}
\begin{aligned}
\sum_{j=2,4} -\text{TFlux}_{1j} = 
...
حال با ترکیب \ارجاع‌فر{eq:3poly15} و  \ارجاع‌فر{eq:3poly11} داریم:
\begin{equation}
\label{eq:3poly16}
\sum_{j=2,4} -\text{TFlux}_{1j} = 
...
از مقایسه دو معادله \ارجاع‌فر{eq:3poly14} و  \ارجاع‌فر{eq:3poly16} مقادیر $a$ و $b$ را برای نود شماره ۱ به دست می‌آید:
\begin{equation}
\label{eq:3poly17}
\begin{bmatrix}
	a_{11} &\ldots &a_{1N} 
...
در آخر نحوه محاسبه حجم $B_i$ها را ذکر می‌کنیم. با استفاده از رابطه \ارجاع‌فر{eq:3poly7} تقریب مقدار میانی برای محاسبه انتگرال‌ها می‌توانیم بنویسیم:
\begin{equation}
\label{eq:3poly19}
...
\زیرقسمت{المان شرایط مرزی}
\begin{figure}
\begin{subfigure}{.5\textwidth}
\includegraphics[width=.9\linewidth,center]{numeric-bnd-flux.eps} 
\شرح{فرض می‌شود شار ورودی به حجم کنترل از یک المان فرضی وارد آن می‌شود}
\label{fig:3bnd-imaj}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=.9\linewidth,center]{numeric-bnd-notation.eps} 
\شرح{نام‌گذاری نودها و حجم کنترل}
\label{fig:3bnd-real}
\end{subfigure}
\شرح{یک نود روی مرز هندسه}
\label{fig:3bnd}
\end{figure}

شکل \رجوع{fig:3bnd-real} یک نود که روی مرز قرار دارد و حجم کنترل متناظرش را نمایش می‌دهد. نام این نود $i$ و نام نود‌های مجاورش که روی مرز قرار دارند $j$ و $k$ می‌باشد. به علاوه نیمی از ضلع $ij$ به ناحیه مرزی $\Gamma_1$ و نیمی  از ضلع $ik$ به ناحیه مرزی $\Gamma_2$ تعلق دارد. هدف ما این است که شاری که از اضلاع $ij$ و $ik$ وارد حجم کنترل می‌شوند را محاسبه کنیم و تعیین کنیم که وجود این شار‌ها چه تغییراتی در
...
\begin{tight_enumerate}
\فقره در هر دو ناحیه مرزی $\Gamma_1$ و $\Gamma_2$ شرط مرزی نفوذناپذیر حاکم باشد. در این حالت هیچ شاری از $ij$ و $ik$ عبور نمی‌کند. لذا این نود تأثیری در $\textbf A_G$، $\vec{\mathcal B}_G$ و $\vec{\mathcal F}_G$ و یا سیال ورودی و خروجی به مخزن نخواهد داشت.

\فقره در نواحی $\Gamma_1$ و $\Gamma_2$ دو شرط مرزی نفوذپذیر و متفاوت حاکم باشد. روش عددی ما قادر به مدل کردن چنین حالتی نیست. لذا در صورت بروز چنین حالتی بهتر است یک ناحیه کوچک نفوذناپذیر بین دو ناحیه $\Gamma_1$ و $\Gamma_2$ ایجاد کنیم.

\فقره $\Gamma_1$ نفوذپذیر باشد و $\Gamma_2$ یا نفوذناپذیر باشد یا همان $\Gamma_1$ باشد. از بین حالات عنوان شده این حالت است که نیاز به اعمال تدابیر ویژه‌ای دارد و در ادامه فقط لازم است به این حالت بپردازیم.
\end{tight_enumerate}

با توجه به فرضیات عنوان شده در حالت سوم، بردار مرزی نود $i$ یا همان $(ln)_i$ را به صورت معادله \ارجاع‌فر{eq:3bnd1} تعریف می‌کنیم.
\begin{equation}
\label{eq:3bnd1}
...
نشان می‌دهیم. مطابق شکل  \رجوع{fig:3bnd-imaj} می‌توانیم فرض کنیم این شار از یک المان مجازی وارد $B_i$ می‌شود.اگر این المان را $e_{bnd}$ بنامیم، با توجه به معادلات \eqref{eq:3fvmp4} و \eqref{eq:3fvms3} داریم:
...
\زیرزیرقسمت*{شرط مرزی نیومن برای پتانسیل}
...
\vec{\mathcal B} = 
	...
	\end{bmatrix}
\end{equation}
\زیرزیرقسمت*{شرط مرزی دیریشله برای پتانسیل}
در این حالت مقدار
...
\زیرزیرقسمت*{شرط مرزی نیومن برای اشباع}
در این حالت مقدار $u_{\Gamma i}$ از قبل توسط یکی از معادلات \eqref{eq:3bnd3} یا \eqref{eq:3bnd3} محاسبه شده است. حال برای محاسبه $u_{w\Gamma i}$ از ترکیب  \ارجاع‌فر{eq:3dim4}، \ارجاع‌فر{eq:3dim5} و \ارجاع‌فر{eq:2bndout} می‌توان نوشت:
\begin{equation}
\label{eq:3bnd6}
...
\زیرزیرقسمت*{شرط مرزی دیریشله برای اشباع}
...
\end{equation}
\زیرزیرقسمت*{محاسبه میزان سیال ورودی به و خروجی از مخزن}
فرض کنید 
...
%----------------------------------------------------------------------------------------ی
%----------------------------------------------------------------------------------------ی
\قسمت{انتخاب گام زمانی}
روش \متن‌لاتین{IMPES} بسیار حساس به گام زمانی است و در صورتی که گام زمانی بیش از حد بزرگ شود، پاسخ‌ها واگرا خواهند شد. لذا از الگوریتم \رجوع{alg:alg1} برای محاسبه گام زمانی استفاده می‌کنیم \مرجع{mont1}. در این این پروژه معمولاً مقادیر $0.01$، $0.005$ و $1.2$ برای مقادیر
...
%----------------------------------------------------------------------------------------ی
\قسمت{جمع‌بندی}
در این فصل در ابتدا معادلات حاکم بر مسئله را به حالت بدون بعد در آوردیم و نشان دادیم که پارامتر‌های بدون بعد حاکم بر مسئله مانند معادله \ارجاع‌فر{eq:3dim7} می‌باشند. پس از آن به بررسی شبکه محاسباتی پرداختیم و تعاریف مورد نیاز مربوط به آن را ارائه دادیم. در ادامه روش‌های \متن‌لاتین{IMPES} و حجم محدود معرفی و نحوه استفاده از آن‌ها برای حل مسئله بیان شد. در این قسمت روش عددی خود را در الگوریتم \رجوع{alg:alg2} خلاصه می‌نماییم. این الگوریتم در قالب یک کد به زبان \متن‌لاتین{C++} پیاده‌سازی شده است. 

برای پیاده‌سازی کد محاسباتی از کتابخا‌نه‌ها و برنامه‌های متن‌باز مختلفی استفاده شده‌است. برای تولید شبکه محاسباتی از نرم‌افزار‌های \متن‌لاتین{Gmsh}\مرجع{gmsh} و \متن‌لاتین{Triangle}\مرجع{triangle} استفاده شده است. برای حل دستگاه معادلات \ارجاع‌فر{eq:3impes1} و ذخیره ماتریس‌های اسپارس از کتابخانه \متن‌لاتین{Petsc}\مرجع{petsc1,petsc2} بهره گرفته‌ شده‌است. این کتابخانه ابزارهای مناسبی برای رفع اشکال کد را نیز در اختیار مؤلف قرار داده‌است. جبر ماتریسی مورد نیاز برای محاسبه ماتریس‌های محلّی المان‌های مثلثی و چهار‌ضلعی به کمک کتابخانه \متن‌لاتین{Armadillo}\مرجع{arma} برنامه‌نویسی شده است. درآخر نرم‌افزارهای \متن‌لاتین{Paraview}\مرجع{paraview}، \متن‌لاتین{Octave}\مرجع{octave} و \متن‌لاتین{VisIt}\مرجع{visit} ابزار مناسب برای رسم گرافیکی پاسخ‌های آمده در فصل \رجوع{ch:fasl4} را فراهم کردند.
...
\begin{algorithmic}[1]      
	...
	\\ هندسه مسئله 
	\\ شبکه محاسباتی 
	\\ شرایط مرزی مطابق معادله \ارجاع‌فر{eq:2bndout} 
	\\ شرایط اوّلیه مطابق معادله \ارجاع‌فر{eq:2ini} 
	\\ اعداد بی بعد مطابق معادله \ارجاع‌فر{eq:3dim7} و جهت جاذبه : $\nabla h$ 
	\\ ویژگی‌های هر ناحیه سنگ مخزن: $\textbf{K}$، $\phi$، $P_d$، $k_{rw}$، $k_{ro}$ و ضخامت نواحی ترک: $e$
	\\ منحنی‌ فشار مویینگی: $J$
	\\ پارامتر‌های کنترل گام زمانی مطابق الگوریتم \رجوع{alg:alg1}
